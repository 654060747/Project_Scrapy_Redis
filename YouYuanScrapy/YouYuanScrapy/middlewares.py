...
class YouyuanscrapySpiderMiddleware:
...
class YouyuanscrapyDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
...
class UserAgentMiddleware(object):
    ...
    def process_request(self, request, spider):
        ...
        if spider.name=='huangye88':
            ...
            # request.headers['Referer'] = "https://so.huangye88.com/?kw=%E7%BB%B4%E4%BF%AE&type=sale"
...
class ProxyMiddleware(object):
    def process_request(self, request, spider):
        ...
        self.verify_net(proxy_ip,request,proxy_list)
    def verify_net(self,proxy_ip,request,proxy_list):
        # 验证代理IP是否适合指定的爬虫网站 格式：HTTP://112.47.3.53:3128
        proxies = {proxy_ip.split("://")[0]:proxy_ip.split("://")[1]}
        myurl = 'http://m.yoyuan.com.cn/find/beijing/mm18-0/advance-0-0-0-0-0-0-0/p1/'
        conn = requests.get(myurl, proxies = proxies, verify=False)
    ...
    def get_random_proxy(self):
        # 需要更换的windows下代理ip路径
        with open('D:\\01llg\\Project_Scrapy_Redis\\YouYuanScrapy\\YouYuanScrapy\\proxy_ip\\verified_proxy.csv') as f:  
        # 读取文件所有行
            proxies = f.readlines()
        return proxies 